...
const fetchOrders = async () => {
  try {
    const { data } = await $axios.get('/api/orders-placed/dispatch', {
      params: {
        page: table.page,
            ...
            <table class="table table-hover table-striped align-middle mb-0 table-sticky">
              ...
              <tbody>
                <tr v-for="(order, index) in orders" :key="order.id">
                  ...
                  <td class="py-2 px-3">
                    <div class="form-check m-0 d-flex align-items-center gap-2">
                     <label class="form-check-label text-muted small" :for="`check-dispatch-${order.id}`">{{ index + 1
                        }}</label>
                    </div>
                  </td>
                  ...
                  <td class="py-2 px-3">
                    <div class="d-flex justify-content-center gap-2">
                      <NuxtLink :to="`/admin/orders/dispatch/${order.id}`" class="btn btn-sm btn-success px-3">View
                      </NuxtLink>
