(function ($) {
  'use strict';
   ...
   // delegate so it works for every depth that exists now or is added later
   $('#sidebar-menu').on('click', 'li.dropdown > a', function (e) {
    e.preventDefault();                            // keep the hash-link from scrolling

    const $li = $(this).parent('li.dropdown');    // clicked <li>

    // toggle just this branch
    $li.children('.sidebar-submenu').slideToggle(200);
    $li.toggleClass('dropdown-open');

    // close **sibling** branches at the same depth
    $li.siblings('.dropdown').removeClass('dropdown-open')
       .children('.sidebar-submenu:visible').slideUp(200);
  });

  /* existing sidebar-wide toggles … */
  $('.sidebar-toggle').on('click', function () {
    $(this).toggleClass('active');
    $('.sidebar, .dashboard-main').toggleClass('active');
  });
  ...
  $(function () {
    ...
    // open the root branches you want visible on first load (optional)
    $menu.find('> li.has-child').addClass('open').children('ul').show();

    // delegate click → branch toggle
    $menu.on('click', 'li.has-child > a', function (e) {
      e.preventDefault();                       // stay on page

      const $branch = $(this).parent('li');

      // Toggle this branch
      $branch.toggleClass('open')
             .children('ul').stop(true, true).slideToggle(180);
    });
  });
